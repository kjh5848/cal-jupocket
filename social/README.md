...
플랫폼마다 하는 일이 다르다. 같은 큐 파일을 쓰되 보는 것이 다르다.

| | 올리는 것 | 빈도 | 병목 |
|---|---|---|---|
| 스레드 | **글만** | 하루 여러 편 | 없음 |
| 인스타 | 카드뉴스 | 하루 1편 | 카드 렌더(수동) |

`threads-post.mjs` 는 `images:` 를 **아예 보지 않는다**. 카드 렌더링이
빠지면 스레드는 하루에 여러 편을 낼 수 있다. 같은 파일의 `images:` 는
`ig-post.mjs` 만 읽고, 이미지가 없는 큐 글은 인스타가 알아서 건너뛴다.
...
```bash
npm run social:post -- --due              # 드라이런
npm run social:post -- --due --publish    # 시각이 된 것 하나만 게시
```

**--due 는 한 번에 하나만, 그것도 시각 직후 90분 안에만 올린다.**

"지났으면 무조건" 으로 만들면 저녁에 처음 돌리는 순간 아침·점심 예약분이
한꺼번에 밀려나와 시간대를 나눈 이유가 사라진다. PC 가 꺼져 있었을 때도
같은 일이 생긴다. 창을 넘긴 글은 오늘은 건너뛰고, `at:` 이 하루 중 시각이라
**내일 그 시간대에 다시 후보가 된다.**
...
스케줄러(켜짐/꺼짐·다음 실행·마지막 결과)와 큐 전체를 한 화면에 보여준다.
게시하지 않고 읽기만 한다. 본문에 링크가 있는 글은 여기서 ✖ 로 뜬다 —
게시 단계에서 거부되므로 미리 안다.

### 자동 실행 (Windows)

`scripts/post-due.cmd` 를 30분마다 돌린다. 등록·중지는 `scripts/README.md`.
**PC 가 켜져 있어야 한다** — 토큰이 로컬 `.env` 에만 있어서 클라우드로 못 옮긴다.

### 하루 여러 편을 쓸 때

글 한 편에서 각도를 나눈다. **표현만 바꾼 중복이 아니라 서로 다른 사실**을
담아야 한다 — 반복 게시로 판단되면 도달이 깎이고, 같은 타임라인을 보는
사람에게도 역효과다.

국민연금 글 한 편에서 나오는 각도 예:
요율 변화 / 직장인 vs 지역가입자 / 기준소득월액 상한·하한 / 적용 시점 /
미납했을 때 / 임의가입 / 추납 — 세금 주제는 사실이 촘촘해서 각도가 나온다.
